...
_libdovi () {
    [ -d libdovi ] || $gitclone https://github.com/quietvoid/dovi_tool.git
    builddir libdovi
    local CARGO_BUILD_TARGET_DIR=$PWD
    cargo cinstall --manifest-path dolby_vision/Cargo.toml --prefix $prefix_dir \
        --target $fam-pc-windows-gnu -Zbuild-std=std,panic_abort \
        -Zbuild-std-features=sysroot/panic_immediate_abort \
        --release --library-type cdylib
    popd
}
...
if [ "$2" = pack ]; then
    ...
    dlls=(
        ...
        lib{shaderc_shared,spirv-cross-c-shared,dav1d,lcms2,libdovi}.dll zlib1.dll
    )
